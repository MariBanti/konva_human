...
export class CreateHumanService {
  ...
  private drawHead(): void {
      ...
      this.addPolygon(
        [
          this.centerX, headTop,
          this.centerX - width / 2, headTop + height / 2,
          this.centerX, headTop + height,
          this.centerX + width / 2, headTop + height / 2,
        ],
    ...
    private drawHat(): void {
      ...
      this.addPolygon(
        [
          this.centerX, headTop,
          this.centerX + 80, headTop + height / 2,
          this.centerX - 80, headTop + height / 2
        ],
    ...
    private drawBody(): void {
      ...
      this.addPolygon(
        [
          this.centerX - body.halfWidth, bodyTop - 10,
          this.centerX + body.halfWidth, bodyTop - 10,
          this.centerX, bodyTop + bodyHeight       
        ],
    ...
    private createLeg(side: Side): void {
      ...
      const legGroup = new Konva.Group({
        x: this.centerX + sign * (legs.legWidth - (side === 'right' ? legs.hipWidth : 0)) + legs.hipWidth / 2,
        y: legTop
      });
      ...
      let shinThirdPointX = legs.hipWidth / 2 - (side === 'right' ? (0.6 * legs.hipWidth) : 0);
      let shinFourthPointX = side === 'right' ? -legs.hipWidth/2 : 0.1 * legs.hipWidth;
      
      const shin = this.createPolygon(
        [
          - legs.hipWidth / 2, 0,
          legs.hipWidth / 2, 0,
          shinThirdPointX, legs.shinLength,
          shinFourthPointX, legs.shinLength,
        ],
        this.config.colors.skin,
        this.config.colors.skinStroke,
        this.config.strokeWidth
      ...
      // Стопа
      const feetFirstPointX = side === 'right' ? shinThirdPointX : shinFourthPointX;
      ...
      const foot = this.createPolygon(
        [
          0, 0,
          sign * footTopWidth, 0,
          sign * footTopWidth, legs.feetLength,
          feetFirstPointX, legs.feetLength,
        ],
    ...
    private createArm(side: Side): void {
      ...
      const sign = side === 'right' ? 1 : -1;
      const shoulderX = this.centerX + sign * (body.halfWidth - (side === 'right' ? arms.shoulderWidth : 0));
      ...
      const forearmX = (side === 'right' ? arms.shoulderWidth : 0) + sign * 0.9 * forearmLength;
  
      const forearm = this.createPolygon(
        [
          - arms.shoulderWidth / 2, 0,
          arms.shoulderWidth / 2, 0,
          forearmX, 0.7 * shoulderLength
        ],
        this.config.colors.skin,
        this.config.colors.skinStroke,
        this.config.strokeWidth,
    ...
    private drawCollar(): void {
      ...
      this.addPolygon(
        [
          this.centerX - body.halfWidth - 5, bodyTop - 10,
          this.centerX + body.halfWidth + 5, bodyTop - 10,
          this.centerX + body.halfWidth + 15, bodyTop + 10,
          this.centerX - body.halfWidth - 15, bodyTop + 10       
        ],
        this.config.colors.purpleDress,
        this.config.colors.purpleDress,
